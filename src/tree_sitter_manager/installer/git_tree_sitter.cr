  ...
  module Installer
    ...
    class GitTreeSitter < Base
      ...
      def download(language : String, temporary_directory : String) : BoolResult
        ...
        clone = Process.run("git", ["clone", "--depth", "1", repository, "."], chdir: temporary_directory)
      ...
      def create_manifest(language : String, temporary_directory : String) : GrammarMetadata
        ...
        GrammarMetadata.new(url: repository_url_for(language), type: "tree-sitter", commit_hash: status.success? ? commit.to_s.strip : nil, package_name: LanguageRegistry.package_name(language) || "tree-sitter-#{language}", language: language)
      end
